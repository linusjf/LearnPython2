...
def testInputCode():
    ...
    assertResponse(resp1, status=200, contains=map.the_bridge.name)

def testTheBridge():
    resp = invokeTheBridge()
    resp = invokePostGame(data={'action':'slowly place the bomb'})
    assertResponse(resp,status=200,contains=map.escape_pod.name)
